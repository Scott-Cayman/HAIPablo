...
### 3.2 为什么 PostgreSQL 更适合

PostgreSQL 是成熟的关系型数据库，适合这个项目的生产阶段，原因是：

1. 支持高并发，多用户同时使用时不会因为文件锁造成等待。
2. 支持跨服务器部署，适合 Docker Compose 或云服务架构。
3. 存储模板配置、任务记录、文件路径、生成参数完全够用。
4. 支持 JSON 字段，可灵活存储半结构化数据（如模板变量、UI 配置）。

不要把图片二进制存在数据库里。原因是：

1. 图片体积大，批量生成后数据库会迅速膨胀，影响备份和恢复效率。
2. 图片读取、预览、下载都会影响数据库性能和连接数。
3. 后续迁移对象存储会更麻烦。

所以最终方案是：

```txt
数据库只存：文件名、文件路径、文件类型、尺寸、大小、上传人、归属项目、生成任务 ID。
...
10. 数据库使用 PostgreSQL + Prisma，图片文件存在本地 storage 目录，数据库只存文件元数据和路径。
...
2. 配置 Tailwind、shadcn/ui、Prisma、PostgreSQL
